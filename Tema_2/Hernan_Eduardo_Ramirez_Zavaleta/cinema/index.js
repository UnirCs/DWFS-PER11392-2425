...
function suggest(cantidad) {
    ...
    for (let i = N - 1; i >= 0; i--) {
        ...
        for (let j = 0; j < N; j++) {
            if (!fila[j].estado) { // Si el asiento está libre
                ...
                // 📌 Si ya encontramos suficientes asientos, terminamos
                if (consecutivos.length === cantidad) {
                    return new Set(consecutivos); // El resultado debe ser un Set con los ids de los asientos pre-seleccionados.
                }
            } else {
    ...
    // Si no encontramos asientos juntos, devolvemos un Set vacío
