...
function greyConverter() {
  ...
  const calcMedia =(rgbArray)=>{
    let suma=0
    for (let i = 0; i < rgbArray.length; i++){
      suma = suma+rgbArray[i] ;
    }
    return suma/rgbArray.length;
  }

  for ( let i = 0 ; i < pixels.length; i++){
    for ( let j = 0; j < pixels[i].length; j++){
      const media= calcMedia(pixels[i][j])
...
function blackAndWhiteConverter() {
  ...
  const calcMediaBW =(rgbArray)=>{
    let suma=0
    for (let i = 0; i < rgbArray.length; i++){
      suma = suma+rgbArray[i] ;
    }
    return suma/rgbArray.length < 128 ? 0: 255;
  }

  for ( let i = 0 ; i < pixels.length; i++){
    for ( let j = 0; j < pixels[i].length; j++){
      const media= calcMediaBW(pixels[i][j])
      for(let k = 0; k < pixels[i][j].length; k++){
          pixels[i][j][k]=media
      }
    }
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      ...
      for (let k = 0; k < catPixels[i][j].length; k++) {
        let catValue = catPixels[i][j][k] * alphaFirst;
        let dogValue = dogPixels[i][j][k] * alphaSecond;
        let auxValue = Math.round(catValue + dogValue);
        pixel.push(auxValue);
      }
      row.push(pixel);
    }
