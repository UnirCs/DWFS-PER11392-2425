...
function redConverter() {
  let outputPath = 'output/tucan_red2.jpg';
...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i+=2) {
    ...
    if (newRow.length > 0) scaledPixels.push(newRow);
  }
  pixels = scaledPixels;
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      for (let k = 0; k < 3; k++) {
        pixels[i][j][k] = Math.round(pixels[i][j][k] / dimFactor);
      }
    }
  }
...
function invertColors() {
  ...
  pixels = pixels.map(row =>
      row.map(([r, g, b]) => [255 - r, 255 - g, 255 - b])
  );
...
function merge(alphaFirst, alphaSecond) {
  ...
  pixels = dogPixels.map((row, i) =>
      row.map(([r1, g1, b1], j) => [
        Math.min(255, Math.round(r1 * alphaFirst + catPixels[i][j][0] * alphaSecond)),
        Math.min(255, Math.round(g1 * alphaFirst + catPixels[i][j][1] * alphaSecond)),
        Math.min(255, Math.round(b1 * alphaFirst + catPixels[i][j][2] * alphaSecond))
      ])
  );
